...
ref_data['SUPPORTSUM'] = ref_data['SUPPORTSUM'].astype('category')

# plotting SUPPORTSUM into a graph
seaborn.countplot(x="SUPPORTSUM", data=ref_data)
plt.xlabel('Total Support Score of Respondents')
plt.title('Support Scores of Respondents in the AddHealth Study')

# plotting GRADEAVE into a graph
seaborn.countplot(x="GRADEAVE", data=ref_data)
plt.xlabel('Grade Average of Respondents')
plt.title('Average Grades for Math, Science, English, and History of Respondents in the AddHealth Study')

# collapse GRADEAVE categories to those who have gotten a 50% and above average and those who didn't
def GRADE_COLLAPSE(row):
    if row['GRADEAVE'] == 4 or row['GRADEAVE'] == 3:
        return 1
    if row['GRADEAVE'] == 2 or row['GRADEAVE'] == 1:
        return 0

ref_data['GRADEAVE'] = ref_data.apply(lambda row: GRADE_COLLAPSE (row), axis=1)

# renaming categorical value SUPPORTSUM
ref_data['SUPPORTSUM'] = ref_data['SUPPORTSUM'].cat.rename_categories(['Not at all', 'Very little', 'Somewhat','Quite a bit'])

# plotting GRADEAVE and SUPPORTSUM
seaborn.catplot(x="SUPPORTSUM", y = "GRADEAVE", data=ref_data, kind="bar", ci=None)
plt.xlabel('Total Support Score of Respondents')
plt.ylabel('Grade Proportion of Respondents')
